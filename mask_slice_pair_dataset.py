...
from utils.EMDataset import CRIME_Dataset, CRIME_Dataset3D, labelGenerator3D
...
class CRIME_Dataset_3D_mask_pair(CRIME_Dataset):
    ...
    def __make_slice_MaskPair_with_label__(self,image, seg_label):
        ...
        if np.random.rand() > 0.68: 
          ...
          dilate_mask     = ndimage.binary_dilation(masked_seg, structure=struct2).astype(masked_seg.dtype)
          bool_mask       = dilate_mask.astype(np.bool)
          #bool_mask       = masked_seg.copy().astype(np.bool)
          ...
          lb_connected    = (sel_idx == selected_sliceSeg_ids[masked_slice_idx]) \
                                          and np.count_nonzero(next_layer_mask) >5
