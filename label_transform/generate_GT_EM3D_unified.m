function [dir_map,depth_map,weight_map]=generate_GT_EM3D_unified(gt_label, downsampling, depth_bins)
% generates 2 channel unitvec gt
    boundaries = [0,1,2,3,4,5,7,9,12,15,19,24,30,37,45,54,Inf];
    annotation = uint16(gt_label);
    if downsampling > 1
    ...
    dir_map = repmat(depth_map, 1, 1, 1, 3);
    size(dir_map)
    ...
    for i = 2:length(ids)
       ...
       [dir_i(:,:,:,1), dir_i(:,:,:,2),dir_i(:,:,:,3)] = my_imgradientxyz(depth_i);

       X = ['processing id = ',num2str(i),' of ' ,num2str(length(ids)), ' objects'];
       ...
       weight_map(annotation_i==1) = 2500 / sqrt(sum(annotation_i(:)));
    end
    
    edges = 1-double(edge(annotation));
    dir_map = dir_map .* repmat(edges, 1, 1, 1,3);
    %depth_map = depth_map .* edges;
    depth_bins = boundaries;
    ...
    %   imagesc(weight_map);
    %   figure(5);
    %   imshow(label2rgb(gt_label, 'jet', 'c','shuffle'))
    %   %imagesc(ss_map);
    % end
end

function  [dir_x,dir_y,dir_z]=my_imgradientxyz(data)
  [h,w,d] =  size(data);
  dir_x = zeros(size(data));
  dir_y = zeros(size(data));
  dir_z = zeros(size(data));
  parfor i =1:d
    slice_im =  squeeze(data(:,:,i));
    if sum(slice_im(:)) > 0
      [dir_x(:,:,i), dir_y(:,:,i)] = imgradientxy(slice_im);
    end
  end
  ...
  parfor i=1:h
     slice_im =squeeze(data(i,:,:));
     if sum(slice_im(:)) >0
      [gx, dir_z(i,:,:)] = imgradientxy(slice_im);
    end
  end
end
